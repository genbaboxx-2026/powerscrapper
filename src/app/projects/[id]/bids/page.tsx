            ...
            <div className="space-y-4">
              {bids.map((bid) => (
                <div
                  key={bid.id}
                  className={`card p-4 ${
                    bid.isMatched ? 'border-2 border-[#0F6E56]' : ''
                  }`}
                >
                  {/* 入札者情報 */}
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {bid.isMatched && (
                          <span className="px-2 py-0.5 bg-[#0F6E56] text-white text-xs rounded">
                            選定済み
                          </span>
                        )}
                      </div>
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-lg font-bold text-[#2C2C2A]">
                      {bid.amount
                  ...
                  {/* メッセージ */}
                  <div className="bg-[#F4F3F0] rounded-lg p-3 mb-3">
                    ...
                    <div className="space-y-2 mb-3">
                      {bid.bidder.coverageAreas.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                          <span className="text-xs text-[#73726C]">対応エリア:</span>
                          {bid.bidder.coverageAreas.map((area) => (
                            <span
                              key={area}
                              className="px-2 py-0.5 bg-[#E1F5EE] text-[#0F6E56] text-xs rounded"
                            >
                              {area}
                            </span>
                          ))}
                      ...
                      {bid.bidder.licenses.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                          <span className="text-xs text-[#73726C]">資格:</span>
                  ...
                  {/* フッター */}
                  <div className="flex items-center justify-between pt-3 border-t border-[#D5D5D0]">
                    <span className="text-xs text-[#73726C]">
